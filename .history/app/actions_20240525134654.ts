...
export async function createReview(prevState: {message: string, type: string}, formData: FormData){
    ...
    if (!parse.success) {
        ...
        return { message: "Failed to create review", type: "error" };
      }
    ...
    try{
        ...
        return {message: `Added review successfully.`, type: "success"}
    }catch(err){
        console.log(err)
        return {message: `Failed to add review.`, type: "error"}
    }
}